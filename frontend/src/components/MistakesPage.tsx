...
export function MistakesPage({
  ...
  const visibleMistakes = useMemo(() => {
    ...
    return mistakes.filter((item) => {
      ...
      const queryHit = !query || item.question.questionMarkdown.toLowerCase().includes(query) || item.conceptNames.some((name) => name.toLowerCase().includes(query));
  ...
  const analysis = useMemo(() => {
    ...
    mistakes.filter((item) => item.status !== "mastered").forEach((item) => {
      ...
      const categoryName = item.category || "待分类";
  ...
  const updateAnswer = (mistake: MistakeItem, answer: QuizAnswer) => {
    setPracticeById((current) => ({ ...current, [mistake.id]: { ...current[mistake.id], active: true, answer } }));
  };

  const startPractice = (mistake: MistakeItem) => {
    setPracticeById((current) => ({ ...current, [mistake.id]: { active: true, submitted: false } }));
  };
  ...
  const submitPractice = (mistake: MistakeItem) => {
    ...
    setPracticeById((state) => ({ ...state, [mistake.id]: { ...state[mistake.id], active: true, submitted: true, correct } }));
  };
  ...
  const resolvePractice = (mistake: MistakeItem, resolution: "understood" | "still_confused") => {
    ...
    setPracticeById((state) => ({ ...state, [mistake.id]: { ...state[mistake.id], resolved: resolution } }));
  };
    ...
    <main className={`mistakes-page ${rightPanelMode !== "none" ? "has-right-panel" : ""}`}>
      <div className="mistakes-main">
        ...
        <section className="mistakes-stat-grid">
          <div><span>全部错题</span><strong>{stats.total}</strong></div>
          <div><span>待复习</span><strong>{stats.active}</strong></div>
          <div><span>今日新增</span><strong>{stats.today}</strong></div>
          <div><span>已掌握</span><strong>{stats.mastered}</strong></div>
        </section>
        ...
        <section className="mistakes-analysis">
          ...
          <div><strong>高频分类</strong><span className="source-chip">{analysis.topCategory ? `${analysis.topCategory[0]} · ${analysis.topCategory[1]}` : "暂无"}</span></div>
          <div><strong>高错误次数</strong><span className="source-chip">{analysis.highWrongCount} 题</span></div>
        </section>
                ...
                <button className={`mistakes-list-card ${selectedMistake?.id === mistake.id ? "active" : ""}`} key={mistake.id} onClick={() => setSelectedId(mistake.id)}>
                  <div className="mistakes-list-card-top">
                    <div className="mistakes-list-title one-line-markdown">
                      <MarkdownRenderer className="markdown-body mistake-title-preview" content={normalizeQuestionPreviewMarkdown(mistake.question.questionMarkdown)} compact renderCodeAsText />
                    </div>
            ...
            {selectedMistake && (
              <article className="panel mistakes-detail">
                ...
                <div className="mistakes-detail-actions">
                  ...
                  {selectedPractice.active && !selectedPractice.submitted && (
                    <button className="primary-button small" disabled={!selectedPractice.answer} onClick={() => submitPractice(selectedMistake)}>提交答案</button>
                  )}
                  {selectedMistake.conceptNames[0] && (
                    <button className={`secondary-button small ${isInReview(selectedMistake.conceptNames[0]) ? "done" : ""}`} onClick={() => onAddReview(selectedMistake.conceptNames[0], "quiz")} disabled={isInReview(selectedMistake.conceptNames[0])}>
